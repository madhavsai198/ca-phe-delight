...
const Admin = () => {
  ...
  const markRead = async (id: string, is_read: boolean) => {
    try {
      await api.put(`/messages/${id}`, { is_read });
      refresh();
    } catch (error) {
    ...
    <>
            ...
            <TabsContent value="menu" className="mt-6">
              ...
              {loading ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {items.map((it) => (
                    <div key={it._id} className="rounded-2xl bg-card border border-border shadow-soft overflow-hidden">
                      ...
                      <div className="p-4 space-y-2">
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => remove(it._id)} className="text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></Button>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {messages.map((m) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" onClick={() => markRead(m._id, !m.is_read)}>
                            <Eye className="h-3.5 w-3.5 mr-1" /> Mark {m.is_read ? "unread" : "read"}
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => removeMessage(m._id)} className="text-destructive hover:text-destructive">
      ...
      <Dialog open={editing !== null} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing?._id ? "Edit item" : "New menu item"}</DialogTitle>
          </DialogHeader>
          {editing && (
